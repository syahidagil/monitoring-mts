      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.length === 0 && (
              <tr><td colSpan={6} className="text-center py-12 text-gray-400 text-sm">Tidak ada data tahsin</td></tr>
            )}
            {filtered.map((r) => (
              <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-sm font-medium text-gray-800">{r.materi}</td>
                ...
                <td className="px-4 py-3 text-xs text-gray-500 max-w-xs truncate">{r.catatan ?? "-"}</td>
