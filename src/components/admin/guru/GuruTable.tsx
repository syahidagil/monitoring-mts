        ...
        <table className="w-full min-w-[720px]">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase">Mapel Ampu</th>
              <th className="text-center px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase">Aksi</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {data.length === 0 && (
              <tr><td colSpan={8} className="text-center py-16 text-gray-400 text-sm">Belum ada data guru</td></tr>
            )}
            {data.map((guru, i) => (
                ...
                <td className="px-4 py-3.5">
                  {guru.guruMapel && guru.guruMapel.length > 0 ? (
                    <div className="flex flex-wrap gap-1">
                      {guru.guruMapel.slice(0, 2).map((gm: any) => (
                        <span key={gm.idGuruMapel} className="text-xs bg-green-50 text-green-700 border border-green-200 px-2 py-0.5 rounded-full font-medium">
                          {gm.mataPelajaran.namaMapel}
                        </span>
                      ))}
                      {guru.guruMapel.length > 2 && (
                    ...
                    <span className="text-xs text-amber-600 bg-amber-50 border border-amber-200 px-2.5 py-1 rounded-full font-medium">Belum di-assign</span>
                  )}
                </td>
                <td className="px-4 py-3.5">
