...
export default function OrangtuaTable({ data }: { data: any[] }) {
  ...
  const filtered = data.filter(
    (o) => o.user.name.toLowerCase().includes(search.toLowerCase()) ||
           o.user.username.toLowerCase().includes(search.toLowerCase())
  );

  function handleDelete(id: string) {
    startTransition(async () => {
      const result = await deleteOrangtua(id);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari nama orang tua..."
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="text-left px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase">Username</th>
              <th className="text-left px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase">No. HP</th>
              <th className="text-left px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase">Anak</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.length === 0 && (
              <tr><td colSpan={6} className="text-center py-12 text-gray-400 text-sm">Belum ada data orang tua</td></tr>
            )}
            {filtered.map((ortu) => (
              <tr key={ortu.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-5 py-4">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-sm font-medium text-gray-800">{ortu.user.name}</span>
                  </div>
                </td>
                <td className="px-5 py-4 text-sm text-gray-500">{ortu.user.username}</td>
                <td className="px-5 py-4 text-sm text-gray-500">{ortu.noHp ?? "-"}</td>
                <td className="px-5 py-4">
                  {ortu.anak.length > 0 ? (
                    <div className="flex flex-col gap-0.5">
                      {ortu.anak.map((a: any) => (
                        <span key={a.id} className="text-xs text-gray-600">{a.nama}</span>
                      ))}
                    </div>
                  ) : <span className="text-xs text-gray-400">-</span>}
                </td>
                <td className="px-5 py-4">
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${ortu.user.status ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                  ...
                  <div className="flex items-center justify-center gap-2">
                    ...
                    {confirmId === ortu.id ? (
                      <div className="flex gap-1">
                        <button onClick={() => handleDelete(ortu.id)} disabled={isPending}
                      ...
                      <button onClick={() => setConfirmId(ortu.id)}
