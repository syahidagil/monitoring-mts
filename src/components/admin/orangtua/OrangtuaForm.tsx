...
type Props = { defaultValues?: any; isEdit?: boolean; ortuId?: string };
...
export default function OrangtuaForm({ defaultValues, isEdit, ortuId }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = isEdit && ortuId ? await updateOrangtua(ortuId, fd) : await createOrangtua(fd);
      if (result.success) router.push("/admin/data-orangtua");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input name="name" defaultValue={defaultValues?.name} required className={inputClass} placeholder="Nama orang tua" />
          </div>
          <div>
          ...
          <div>
            ...
            <input name="pekerjaan" defaultValue={defaultValues?.pekerjaan} className={inputClass} placeholder="Contoh: Wiraswasta" />
          </div>
          <div>
      ...
      <div className="flex items-center justify-between">
        <Link href="/admin/data-orangtua" className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
        ...
        <button type="submit" disabled={isPending}
          ...
          {isPending ? "Menyimpan..." : isEdit ? "Simpan Perubahan" : "Tambah Orang Tua"}
        </button>
      </div>
    </form>
  );
