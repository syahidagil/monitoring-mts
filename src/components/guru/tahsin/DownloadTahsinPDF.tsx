...
import type { SiswaTahsin } from "@/actions/guru/tahsin.action";
...
export default function DownloadTahsinPDF({ data, guruNama, namaKelas, tahunAjaran }: Props) {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      doc.text("REKAP PROGRESS TAHSIN AL-QUR'AN", pageW / 2, 50, { align: "center" });

      doc.setLineWidth(0.4);
      doc.setDrawColor(27, 94, 32);
      doc.line(pageW / 2 - 52, 52, pageW / 2 + 52, 52);
      ...
      autoTable(doc, {
        startY: tableStartY,
        head: [["No", "Nama Siswa", "NIS", "Kelas", "Juz", "Halaman", "Surat", "Total Setoran", "Setoran Terakhir"]],
        body: rows,
      ...
      doc.text(
        `Total: ${data.length} siswa  •  Sudah setoran: ${sudahSetoran}  •  Belum setoran: ${data.length - sudahSetoran}`,
        margin, finalY
      );
      ...
      const fileName = `Rekap_Tahsin_${namaKelas.replace(/\s/g, "_")}_${new Date().toISOString().split("T")[0]}.pdf`;
      doc.save(fileName);
    } finally {
      setLoading(false);
    }
