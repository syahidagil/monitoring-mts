...
export default async function EditSiswaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [siswa, kelas, orangtua, tahunAjaran] = await Promise.all([
    prisma.siswa.findUnique({ where: { id: Number(id) } }),
    prisma.kelas.findMany({ include: { tahunAjaran: true }, orderBy: [{ tingkat: "asc" }, { nama: "asc" }] }),
    prisma.orangTua.findMany({ include: { user: { select: { name: true } } }, orderBy: { user: { name: "asc" } } }),
    prisma.tahunAjaran.findMany({ orderBy: { nama: "asc" } }),
  ]);
  ...
  return (
    <div className="space-y-5">
      ...
      <SiswaForm kelas={kelas} orangtua={orangtua} tahunAjaran={tahunAjaran} defaultValues={siswa} isEdit siswaId={siswa.id} />
    </div>
  );
}