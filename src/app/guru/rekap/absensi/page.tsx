...
import DownloadRekapAbsensiPDF from "@/components/guru/rekap/DownloadRekapAbsensiPDF";
...
export default async function RekapAbsensiPage({ searchParams }: Props) {
  ...
  const [data, kelasList, guru] = await Promise.all([
  ...
  const kelasFiler = kelasList.find((k) => k.id === kelasId)?.nama;
      ...
      <div className="max-w-5xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          ...
          <DownloadRekapAbsensiPDF
            ...
            tanggalMulai={params.mulai}
            tanggalAkhir={params.akhir}
            kelasFiler={kelasFiler ? `Kelas ${kelasFiler}` : undefined}
          />
        </div>
